...
| | Claude (родной) | Codex | ChatGPT | Gemini | Cursor |
|---|---|---|---|---|---|
| Тело логики (процесс, пороги, гейты, safety) | да | да | да | да | да |
| Запуск Python-скриптов | да | да | только с Code Interpreter | да | да |
| Установка | плагин-маркетплейс | `AGENTS.md` в репо | Instructions в Custom GPT | `GEMINI.md` в репо | `*.mdc` в `.cursor/rules/` |
