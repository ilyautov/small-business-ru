...
Опции:
    ...
    --vznosy-uplacheno  сколько взносов «за себя» уже уплачено, ₽ (по умолчанию 0).
                        На вычет НЕ влияет (с 2025 фикс. взносы уменьшают аванс
                        начислением), но уменьшает «взносы_к_доплате» и «итого_к_доплате»
...
# Зеркало канона на случай, если tax_data.py недоступен. Сверяется с каноном
# в eval (run_eval.py, проверка mirror-sync) — при обновлении канона CI упадёт,
# пока зеркало не обновят вместе с ним.
_MIRROR_2026 = {"fix": 57390, "cap": 321818, "porog_1pct": 300_000,
...
try:
    ...
    _C2026 = dict(_MIRROR_2026)
    _CANON = "встроенное зеркало канона (tax_data.py не найден)"

# Ставки из НК РФ (стабильные, в канон tax_data не вынесены):
USN6_RATE = 0.06        # УСН «Доходы»
USN15_RATE = 0.15       # УСН «Доходы минус расходы»
USN_MIN_RATE = 0.01     # минимальный налог УСН-15, % от доходов
EXTRA_1PCT_RATE = 0.01  # доп. взнос ИП с дохода свыше порога
...
def calc(year, rejim, dohod, rashod, rabotniki, vznosy_uplacheno, vznosy_rabotniki=0):
    warn = []
    if dohod < 0:
        return {"ошибка": f"доход не может быть отрицательным (получено {dohod})"}
    if rashod < 0:
        return {"ошибка": f"расход не может быть отрицательным (получено {rashod})"}
    if vznosy_uplacheno < 0 or vznosy_rabotniki < 0:
        return {"ошибка": "взносы не могут быть отрицательными"}
    if rabotniki not in (0, 1):
        return {"ошибка": f"rabotniki: 0 | 1 (получено {rabotniki})"}
    if year not in FIX_VZNOS:
        return {"ошибка": f"нет выверенных ставок за {year} (есть 2024-2026)"}

    fix = FIX_VZNOS[year]
    if year != 2026:
        warn.append(f"пороги НДС/лимита УСН в расчёте — из канона 2026; для {year} сверьте пороги с бухгалтером")
    ...
    vznos_1pct_raw = round(baza_1pct * EXTRA_1PCT_RATE)
    cap = CAP_1PCT[year]
    ...
    # налог УСН
    if rejim == "usn6":
        nalog_do_vycheta = round(dohod * USN6_RATE)
        ...
        if rashod > dohod:
            warn.append(f"расходы ({rashod:,} ₽) превышают доходы ({dohod:,} ₽) — убыток; налоговая база 0, но минимальный налог 1% от доходов платится всё равно".replace(",", " "))
        nalog_raschet = round(baza * USN15_RATE)
        min_nalog = round(dohod * USN_MIN_RATE)
    ...
    # уже уплаченные взносы «за себя»: на вычет не влияют (с 2025 уменьшение —
    # начислением), но остаток к доплате обязан их учитывать
    vznosy_k_doplate = max(0, vznosy_vsego - vznosy_uplacheno)
    if vznosy_uplacheno > vznosy_vsego:
        warn.append(f"взносов уплачено ({vznosy_uplacheno:,} ₽) больше, чем начислено ({vznosy_vsego:,} ₽) — переплата, сверьте с ЛК ФНС".replace(",", " "))
    ...
    return {
        ...
        "взносы_уплачено": rub(vznosy_uplacheno),
        "взносы_к_доплате": rub(vznosy_k_doplate),
        ...
        "итого_к_доплате": rub(nalog_k_uplate + vznosy_k_doplate),
        "ндс": nds,
        "предупреждения": warn,
...
def main():
    ...
    if "ошибка" in res:
        print(json.dumps(res, ensure_ascii=False) if a.json else f"ошибка: {res['ошибка']}",
              file=sys.stderr)
        sys.exit(1)
    if a.json:
